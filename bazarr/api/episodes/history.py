...
class EpisodesHistory(Resource):
    ...
    def get(self):
        ...
        if settings.general.getboolean("upgrade_subs"):
            ...
            minimum_timestamp = (
                (datetime.datetime.now() - timedelta(days=int(days_to_upgrade_subs)))
            ...
            upgradable_episodes = (
                ...
                .join(
                    TableEpisodes,
                    on=(TableHistory.sonarrEpisodeId == TableEpisodes.sonarrEpisodeId),
                )
                .join(
                    TableShows,
                    on=(TableHistory.sonarrSeriesId == TableShows.sonarrSeriesId),
                )
        ...
        for item in episode_history:
            ...
            } in upgradable_episodes_not_perfect:  # noqa: E129
                if os.path.exists(
                    path_mappings.path_replace(item["subtitles_path"])
                ) and os.path.exists(path_mappings.path_replace(item["video_path"])):
        ...
        count = (
            ...
            .where(TableEpisodes.title.is_null(False))
            .count()
        )
