...
class PlexLibraries(Resource):
    def get(self):
        ...
        try:
            ...
            decrypted_token = get_decrypted_token()
            if not decrypted_token:
                logging.warning("No valid token available for library fetch")
                return {'data': []}
            ...
            # Make the API call to get library sections
            headers = {
                'X-Plex-Token': decrypted_token,
                'Accept': 'application/json'
            }
            ...
            for section in sections:
                if isinstance(section, dict):
                    ...
                    try:
                        ...
                        logging.debug(f"Failed to get library count for {section.get('title')}: {e}")
            ...
            logging.info(f"Successfully fetched {len(libraries)} libraries from Plex server")
