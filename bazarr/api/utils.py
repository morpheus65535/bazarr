...
def postprocessEpisode(item):
    ...
    if "audio_language" in item and item["audio_language"] is not None:
        item["audio_language"] = get_audio_profile_languages(
            episode_id=item["sonarrEpisodeId"]
        )
        ...
        for i, subs in enumerate(item["missing_subtitles"]):
            ...
            if len(subtitle) > 1:
                item["missing_subtitles"][i].update(
                    {"forced": subtitle[1] == "forced", "hi": subtitle[1] == "hi"}
                )
...
def postprocessMovie(item):
        ...
        for i, subs in enumerate(item["subtitles"]):
            ...
            if len(language) > 1:
                item["subtitles"][i].update(
                    {"forced": language[1] == "forced", "hi": language[1] == "hi"}
                )
    ...
    if "subtitles_path" in item:
        ...
        item["subtitles_path"] = path_mappings.path_replace_movie(
            item["subtitles_path"]
        )
