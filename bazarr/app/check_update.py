...
def check_if_new_update():
    if settings.general.branch == "master":
        ...
        logging.error(
            f"BAZARR unknown branch provided to updater: {settings.general.branch}"
        )
    ...
    if not args.no_update:
        ...
        if release:
            ...
            try:
                ...
                new_version = (
                    semver.compare(
                        release["name"].lstrip("v"), os.environ["BAZARR_VERSION"]
                    )
                    > 0
                )
            ...
            if new_version and release["name"] != "v0.9.1.1":
                logging.debug(
                    f"BAZARR newer release available and will be downloaded: {release['name']}"
                )
                ...
                if current_version.prerelease and not use_prerelease:
                    logging.debug(
                        f"BAZARR previous stable version will be downloaded: {release['name']}"
                    )
...
def update_cleaner(zipfile, bazarr_dir, config_dir):
    ...
    logging.debug(
        f"BAZARR upgrade leftover cleaner will ignore directories matching this "
    ...
    logging.debug(
        f"BAZARR upgrade leftover cleaner will ignore those files: {', '.join(file_to_ignore)}"
    )
    ...
    for foldername, subfolders, filenames in os.walk(bazarr_dir):
        ...
        if (
            not dir_to_ignore_regex.findall(relative_foldername)
            and relative_foldername not in folder_list
        ):
