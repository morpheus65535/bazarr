...
class Scheduler:
    ...
    def add_job(
        self, job, name=None, max_instances=1, coalesce=True, args=None, kwargs=None
    ):
    ...
    def __automatic_backup(self):
        ...
        if backup == "Daily":
            self.aps_scheduler.add_job(
                backup_to_zip,
                CronTrigger(hour=settings.backup.hour),
                max_instances=1,
                coalesce=True,
                misfire_grace_time=15,
                id="backup",
                name="Backup database and configuration file",
                replace_existing=True,
            )
        elif backup == "Weekly":
            self.aps_scheduler.add_job(
                backup_to_zip,
                CronTrigger(day_of_week=settings.backup.day, hour=settings.backup.hour),
                max_instances=1,
                coalesce=True,
                misfire_grace_time=15,
                id="backup",
        ...
        if settings.general.getboolean("use_sonarr"):
            ...
            if full_update == "Daily":
                ...
                self.aps_scheduler.add_job(
                    update_all_episodes,
                    CronTrigger(
                        day_of_week=settings.sonarr.full_update_day,
        ...
        if settings.general.getboolean("use_radarr"):
            ...
            if full_update == "Daily":
                ...
                self.aps_scheduler.add_job(
                    update_all_movies,
                    CronTrigger(
                        day_of_week=settings.radarr.full_update_day,
    ...
    def __search_wanted_subtitles_task(self):
        if settings.general.getboolean("use_sonarr"):
            self.aps_scheduler.add_job(
                wanted_search_missing_subtitles_series,
                IntervalTrigger(hours=int(settings.general.wanted_search_frequency)),
                max_instances=1,
                coalesce=True,
                misfire_grace_time=15,
    ...
    def __upgrade_subtitles_task(self):
        if settings.general.getboolean("upgrade_subs") and (
            settings.general.getboolean("use_sonarr")
            or settings.general.getboolean("use_radarr")
        ):
            self.aps_scheduler.add_job(
                upgrade_subtitles,
...
if "BAZARR_AUDIO_PROFILES_MIGRATION" in os.environ:
    if settings.general.getboolean("use_sonarr"):
        scheduler.aps_scheduler.modify_job(
            "update_series", next_run_time=datetime.now()
        )
        scheduler.aps_scheduler.modify_job(
            "sync_episodes", next_run_time=datetime.now()
        )
    if settings.general.getboolean("use_radarr"):
        scheduler.aps_scheduler.modify_job(
            "update_movies", next_run_time=datetime.now()
        )
