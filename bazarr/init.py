        ...
        try:
            ...
            if os.path.expanduser("~") == "/":
                ...
                try:
                    ...
                    logging.exception(
                        f"BAZARR requirements.txt installation result: {e.stdout}"
                    )
                    os._exit(1)
                else:
...
if settings.analytics.visitor:
    with open(
        os.path.normpath(os.path.join(args.config_dir, "config", "analytics.dat")), "w+"
    ) as handle:
        handle.write(settings.analytics.visitor)
    with open(
        os.path.normpath(os.path.join(args.config_dir, "config", "config.ini")), "w+"
    ) as handle:
...
def init_binaries():
    try:
        ...
        try:
            ...
            logging.exception(
                "BAZARR requires a rar archive extraction utilities (unrar, unar) and it can't be found."
            )
