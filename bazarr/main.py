...
def web_assets(filename):
    ...
    path = os.path.realpath(
        os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "frontend", "build", "assets"
        )
    )
...
def series_images(url):
    ...
    if get_sonarr_info.is_legacy():
        url_image = f"{url_sonarr()}/api/{url.lstrip(baseUrl)}?apikey={apikey}".replace(
            "poster-250", "poster-500"
        )

    else:
        url_image = (
            f"{url_sonarr()}/api/v3/{url.lstrip(baseUrl)}?apikey={apikey}".replace(
                "poster-250", "poster-500"
            )
        )
    ...
    try:
        req = requests.get(
            url_image, stream=True, timeout=15, verify=False, headers=headers
        )
    except Exception:
        return "", 404
    else:
...
def proxy(protocol, url):
    ...
    try:
        result = requests.get(
            url, params, allow_redirects=False, verify=False, timeout=5, headers=headers
        )
