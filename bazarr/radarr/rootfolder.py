...
def get_radarr_rootfolder():
    ...
    if get_radarr_info.is_legacy():
        url_radarr_api_rootfolder = (
            f"{url_radarr()}/api/rootfolder?apikey={apikey_radarr}"
        )

    else:
        url_radarr_api_rootfolder = (
            f"{url_radarr()}/api/v3/rootfolder?apikey={apikey_radarr}"
        )

    try:
        rootfolder = requests.get(
            url_radarr_api_rootfolder, timeout=60, verify=False, headers=headers
        )
    except requests.exceptions.ConnectionError:
        logging.exception(
            "BAZARR Error trying to get rootfolder from Radarr. Connection Error."
        )
        return []
    except requests.exceptions.Timeout:
        logging.exception(
            "BAZARR Error trying to get rootfolder from Radarr. Timeout Error."
        )
        return []
        ...
        radarr_rootfolder.extend(
            {"id": folder["id"], "path": folder["path"]}
            for folder in rootfolder.json()
            if any(
                item["path"].startswith(folder["path"]) for item in radarr_movies_paths
            )
        )

        db_rootfolder = TableMoviesRootfolder.select(
            TableMoviesRootfolder.id, TableMoviesRootfolder.path
        ).dicts()
        rootfolder_to_remove = [
            x
            for x in db_rootfolder
            if not next(
                (item for item in radarr_rootfolder if item["id"] == x["id"]), False
            )
        ]
        rootfolder_to_update = [
            x
            for x in radarr_rootfolder
        ...
        rootfolder_to_insert = [
            ...
            for x in radarr_rootfolder
            if not next(
                (item for item in db_rootfolder if item["id"] == x["id"]), False
            )
        ]

        for item in rootfolder_to_remove:
            TableMoviesRootfolder.delete().where(
                TableMoviesRootfolder.id == item["id"]
            ).execute()
        for item in rootfolder_to_update:
            TableMoviesRootfolder.update(
                {TableMoviesRootfolder.path: item["path"]}
            ).where(TableMoviesRootfolder.id == item["id"]).execute()
        for item in rootfolder_to_insert:
            TableMoviesRootfolder.insert(
                {
                    TableMoviesRootfolder.id: item["id"],
                    TableMoviesRootfolder.path: item["path"],
                }
...
def check_radarr_rootfolder():
    ...
    rootfolder = TableMoviesRootfolder.select(
        TableMoviesRootfolder.id, TableMoviesRootfolder.path
    ).dicts()
    for item in rootfolder:
        ...
        if not os.path.isdir(path_mappings.path_replace_movie(root_path)):
            TableMoviesRootfolder.update(
                {
                    TableMoviesRootfolder.accessible: 0,
            ...
            TableMoviesRootfolder.update(
                {TableMoviesRootfolder.accessible: 1, TableMoviesRootfolder.error: ""}
            ).where(TableMoviesRootfolder.id == item["id"]).execute()
