...
def update_movies(send_event=True):
    ...
    if apikey_radarr is not None:
        audio_profiles = get_profile_list()
        tagsDict = get_tags()

        if not (
            movies := get_movies_from_radarr_api(
                url=url_radarr(), apikey_radarr=apikey_radarr
            )
        ):
            return
        # Get current movies in DB
        current_movies_db = TableMovies.select(
            TableMovies.tmdbId, TableMovies.path, TableMovies.radarrId
        ).dicts()
        ...
        for i, movie in enumerate(movies):
            ...
            if (
                movie["hasFile"] is True
                and "movieFile" in movie
                and movie["movieFile"]["size"] > 20480
            ):
        ...
        for removed_movie in removed_movies:
            try:
                TableMovies.delete().where(
                    TableMovies.tmdbId == removed_movie
                ).execute()
        ...
        movies_in_db_list.extend(iter(movies_in_db))
        ...
        for updated_movie in movies_to_update_list:
            try:
                TableMovies.update(updated_movie).where(
                    TableMovies.tmdbId == updated_movie["tmdbId"]
                ).execute()
        ...
        for added_movie in movies_to_add:
            try:
                result = (
                    TableMovies.insert(added_movie)
        ...
        for i, altered_movie in enumerate(altered_movies, 1):
            store_subtitles_movie(
                altered_movie[1], path_mappings.path_replace_movie(altered_movie[1])
            )
...
def update_one_movie(movie_id, action, defer_search=False):
    ...
    try:
        ...
        movie_data = get_movies_from_radarr_api(
            url=url_radarr(), apikey_radarr=settings.radarr.apikey, radarr_id=movie_id
        )
        if not movie_data:
            return
        else:
    ...
    # Update existing movie in DB
        ...
        try:
            TableMovies.update(movie).where(
                TableMovies.radarrId == movie["radarrId"]
            ).execute()
        except IntegrityError as e:
            logging.error(
    ...
    logging.debug(
        "BAZARR storing subtitles for this movie: {}".format(
            path_mappings.path_replace_movie(movie["path"])
        )
    )
    store_subtitles_movie(
        movie["path"], path_mappings.path_replace_movie(movie["path"])
    )
