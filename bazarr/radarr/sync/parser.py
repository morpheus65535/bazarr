...
def movieParser(movie, action, tags_dict, movie_default_profile, audio_profiles):
    ...
    try:
        ...
        poster = (
            f"{os.path.splitext(poster_big)[0]}-500{os.path.splitext(poster_big)[1]}"
        )
    ...
    try:
        ...
        try:
            resolution = (
                str(movie["movieFile"]["quality"]["quality"]["resolution"]) + "p"
            )
        except Exception:
            resolution = None
    ...
    if get_radarr_info.is_legacy():
        if (
            ...
            and "audioLanguages" in movie["movieFile"]["mediaInfo"]
        ):
            audio_languages_list = movie["movieFile"]["mediaInfo"][
                "audioLanguages"
            ].split("/")
            if len(audio_languages_list):
                audio_language.extend(
                    audio_language_list.strip()
