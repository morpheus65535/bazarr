...
from app.database import TableBlacklist, database, insert, delete
from app.event_handler import event_stream


def get_blacklist():
    return [(item.provider, item.subs_id) for item in
            database.query(TableBlacklist.provider, TableBlacklist.subs_id).all()]
...
def blacklist_delete(provider, subs_id):
    database.execute(delete(TableBlacklist).where((TableBlacklist.provider == provider) and
