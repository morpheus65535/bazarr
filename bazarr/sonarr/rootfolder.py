...
def get_sonarr_rootfolder():
    ...
    try:
        rootfolder = requests.get(
            url_sonarr_api_rootfolder, timeout=60, verify=False, headers=headers
        )
        ...
        sonarr_rootfolder.extend(
            {"id": folder["id"], "path": folder["path"]}
            for folder in rootfolder.json()
            if any(
                item["path"].startswith(folder["path"]) for item in sonarr_movies_paths
            )
        )

        db_rootfolder = TableShowsRootfolder.select(
            TableShowsRootfolder.id, TableShowsRootfolder.path
        ).dicts()
        ...
        for item in rootfolder_to_remove:
            TableShowsRootfolder.delete().where(
                TableShowsRootfolder.id == item["id"]
            ).execute()
        for item in rootfolder_to_update:
            TableShowsRootfolder.update(
                {TableShowsRootfolder.path: item["path"]}
            ).where(TableShowsRootfolder.id == item["id"]).execute()
        for item in rootfolder_to_insert:
            TableShowsRootfolder.insert(
                {
                    TableShowsRootfolder.id: item["id"],
                    TableShowsRootfolder.path: item["path"],
                }
...
def check_sonarr_rootfolder():
    ...
    rootfolder = TableShowsRootfolder.select(
        TableShowsRootfolder.id, TableShowsRootfolder.path
    ).dicts()
    for item in rootfolder:
        ...
        if not os.path.isdir(path_mappings.path_replace(root_path)):
            TableShowsRootfolder.update(
                {
                    TableShowsRootfolder.accessible: 0,
            ...
            TableShowsRootfolder.update(
                {TableShowsRootfolder.accessible: 1, TableShowsRootfolder.error: ""}
            ).where(TableShowsRootfolder.id == item["id"]).execute()
