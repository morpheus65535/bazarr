...
from .utils import (
    ...
    get_episodesFiles_from_sonarr_api,
)
...
def sync_episodes(series_id=None, send_event=True):
    ...
    seriesIdList = get_series_from_sonarr_api(
        url=url_sonarr(), apikey_sonarr=apikey_sonarr, sonarr_series_id=series_id
    )
    ...
    for i, seriesId in enumerate(seriesIdList):
        ...
        episodes = get_episodes_from_sonarr_api(
            url=url_sonarr(), apikey_sonarr=apikey_sonarr, series_id=seriesId["id"]
        )
        if not episodes:
            continue
        else:
    ...
    for removed_episode in removed_episodes:
        ...
        try:
            TableEpisodes.delete().where(
                TableEpisodes.sonarrEpisodeId == removed_episode
            ).execute()
        except Exception as e:
            logging.error(
                f"BAZARR cannot delete episode {episode_to_delete['path']} because of {e}"
            )
            continue
        else:
            if send_event:
    ...
    for updated_episode in episodes_to_update_list:
        try:
            ...
            logging.error(
                f"BAZARR cannot update episode {updated_episode['path']} because of {e}"
            )
            continue
        else:
    ...
    for added_episode in episodes_to_add:
        try:
            result = (
                TableEpisodes.insert(added_episode)
                .on_conflict(action="IGNORE")
                .execute()
            )
        except IntegrityError as e:
            logging.error(
                f"BAZARR cannot insert episode {added_episode['path']} because of {e}"
            )
            continue
        else:
            if result > 0:
                ...
                if send_event:
                    event_stream(
                        type="episode", payload=added_episode["sonarrEpisodeId"]
                    )
            else:
                logging.debug(
                    "BAZARR unable to insert this episode into the database:{}".format(
    ...
    for i, altered_episode in enumerate(altered_episodes, 1):
        store_subtitles(
            altered_episode[1], path_mappings.path_replace(altered_episode[1])
        )
...
def sync_one_episode(episode_id, defer_search=False):
    logging.debug(
        "BAZARR syncing this specific episode from Sonarr: {}".format(episode_id)
    )
    url = url_sonarr()
    apikey_sonarr = settings.sonarr.apikey
    ...
    existing_episode = (
        TableEpisodes.select(TableEpisodes.path, TableEpisodes.episode_file_id)
    ...
    logging.debug(
        "BAZARR storing subtitles for this episode: {}".format(
            path_mappings.path_replace(episode["path"])
        )
    )
    ...
    if defer_search:
        logging.debug(
            "BAZARR searching for missing subtitles is deferred until scheduled task execution for this "
