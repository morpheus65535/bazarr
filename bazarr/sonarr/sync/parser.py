...
def profile_id_to_language(id_, profiles):
    return [profile[1] for profile in profiles if id_ == profile[0]]


def episodeParser(episode):
    if "hasFile" not in episode:
        return
    if (
        episode["hasFile"] is True
        and "episodeFile" in episode
        and episode["episodeFile"]["size"] > 20480
    ):
        ...
        try:
            video_format, video_resolution = episode["episodeFile"]["quality"][
                "quality"
            ]["name"].split("-")
        except Exception:
            ...
            try:
                video_resolution = (
                    str(episode["episodeFile"]["quality"]["quality"]["resolution"])
                    + "p"
                )
            except Exception:
