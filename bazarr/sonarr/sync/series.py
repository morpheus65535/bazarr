...
def update_series(send_event=True):
    ...
    if not (
        series := get_series_from_sonarr_api(
            url=url_sonarr(), apikey_sonarr=apikey_sonarr
        )
    ):
        return
    ...
    for series in removed_series:
        try:
            ...
            logging.error(
                f"BAZARR cannot delete series with sonarrSeriesId {series} because of {e}"
            )
            continue
        else:
            if send_event:
    ...
    series_in_db_list.extend(iter(series_in_db))
    ...
    for updated_series in series_to_update_list:
        try:
            TableShows.update(updated_series).where(
                TableShows.sonarrSeriesId == updated_series["sonarrSeriesId"]
            ).execute()
        except IntegrityError as e:
            logging.error(
                f"BAZARR cannot update series {updated_series['path']} because of {e}"
            )
            continue
        else:
            if send_event:
    ...
    for added_series in series_to_add:
        try:
            result = (
                TableShows.insert(added_series).on_conflict(action="IGNORE").execute()
            )
        except IntegrityError as e:
            logging.error(
                f"BAZARR cannot insert series {added_series['path']} because of {e}"
            )
            continue
        else:
            if result:
            ...
            if send_event:
                event_stream(
                    type="series",
                    action="update",
                    payload=added_series["sonarrSeriesId"],
                )
...
def update_one_series(series_id, action):
    ...
    if action == "deleted" and existing_series:
        try:
            TableShows.delete().where(
                TableShows.sonarrSeriesId == int(series_id)
            ).execute()
        except Exception as e:
            logging.error(
                f"BAZARR cannot delete series with sonarrSeriesId {series_id} because of {e}"
            )
        else:
            TableEpisodes.delete().where(
                TableEpisodes.sonarrSeriesId == int(series_id)
            ).execute()
    ...
    try:
        ...
        if not (
            series_data := get_series_from_sonarr_api(
                url=url_sonarr(),
                apikey_sonarr=settings.sonarr.apikey,
    ...
    if action == "updated":
        if existing_series:
            try:
                TableShows.update(series).where(
                    TableShows.sonarrSeriesId == series["sonarrSeriesId"]
                ).execute()
            except IntegrityError as e:
                logging.error(
                    f"BAZARR cannot update series {series['path']} because of {e}"
                )
            else:
                sync_episodes(series_id=int(series_id), send_event=True)
                event_stream(type="series", action="update", payload=int(series_id))
                logging.debug(
                    "BAZARR updated this series into the database:{}".format(
                        path_mappings.path_replace(series["path"])
                    )
                )
            ...
            try:
                ...
                logging.error(
                    f"BAZARR cannot insert series {series['path']} because of {e}"
                )
            else:
                event_stream(type="series", action="update", payload=int(series_id))
                logging.debug(
                    "BAZARR inserted this series into the database:{}".format(
                        path_mappings.path_replace(series["path"])
                    )
                )
