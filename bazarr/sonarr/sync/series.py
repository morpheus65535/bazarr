...
def update_series(send_event=True):
    ...
    series_in_db_list = list(series_in_db)
...
def update_one_series(series_id, action):
    ...
    try:
        ...
        if not series_data:
            return
        if action == 'updated':
            series = seriesParser(series_data, action='update', tags_dict=tagsDict, serie_default_profile=serie_default_profile, audio_profiles=audio_profiles) if existing_series else seriesParser(series_data, action='insert', tags_dict=tagsDict, serie_default_profile=serie_default_profile, audio_profiles=audio_profiles)
