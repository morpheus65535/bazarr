...
def store_subtitles_movie(original_path, reversed_path, use_cache=True):
    ...
    if os.path.exists(reversed_path):
        if settings.general.getboolean("use_embedded_subs"):
            ...
            if (
                item := TableMovies.select(
                    TableMovies.movie_file_id, TableMovies.file_size
                )
        ...
        try:
            ...
            subtitles = search_external_subtitles(
                reversed_path, languages=get_language_set()
            )
        ...
        TableMovies.update({TableMovies.subtitles: str(actual_subtitles)}).where(
            TableMovies.path == original_path
        ).execute()
        matching_movies = (
            TableMovies.select(TableMovies.radarrId)
            .where(TableMovies.path == original_path)
            .dicts()
        )
...
def list_missing_subtitles_movies(no=None, send_event=True):
    ...
    for movie_subtitles in movies_subtitles:
                ...
                for language in desired_subtitles_temp["items"]:
                    if language["audio_exclude"] == "True":
                        if language_from_alpha2(
                            language["language"]
                        ) in ast.literal_eval(movie_subtitles["audio_language"]):
                            continue
        ...
        ).where(TableMovies.radarrId == movie_subtitles["radarrId"]).execute()
