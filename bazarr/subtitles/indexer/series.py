...
def store_subtitles(original_path, reversed_path, use_cache=True):
    ...
    if os.path.exists(reversed_path):
        if settings.general.getboolean("use_embedded_subs"):
            ...
            if (
                item := TableEpisodes.select(
                    TableEpisodes.episode_file_id, TableEpisodes.file_size
                )
                ...
                try:
                    ...
                    for (
                        ...
                        try:
                            if (
                                ...
                                or (
                                    settings.general.getboolean("ignore_vobsub_subs")
                                ...
                                logging.debug(
                                    f"BAZARR skipping {subtitle_codec} sub for language: {alpha2_from_alpha3(subtitle_language)}"
                                )
                    ...
                    logging.exception(
                        f"BAZARR error when trying to analyze this {os.path.splitext(reversed_path)[1]} file: {reversed_path}"
                    )
            ...
            for subtitle, language in subtitles.items():
                ...
                if custom is not None:
                    actual_subtitles.append(
                        [custom, path_mappings.path_replace_reverse(subtitle_path)]
                    )

                elif str(language) != "und":
                    if language.forced or not language.hi:
                        language_str = str(language)
                    else:
                        language_str = f"{str(language)}:hi"
        ...
        TableEpisodes.update({TableEpisodes.subtitles: str(actual_subtitles)}).where(
            TableEpisodes.path == original_path
        ).execute()
        matching_episodes = (
            TableEpisodes.select(
                TableEpisodes.sonarrEpisodeId, TableEpisodes.sonarrSeriesId
            )
            .where(TableEpisodes.path == original_path)
            .dicts()
        )
...
def list_missing_subtitles(no=None, epno=None, send_event=True):
    ...
    episodes_subtitles = (
        ...
        .join(
            TableShows, on=(TableEpisodes.sonarrSeriesId == TableShows.sonarrSeriesId)
        )
    ...
    for episode_subtitles in episodes_subtitles:
        ...
        if episode_subtitles["profileId"]:
                ...
                for language in desired_subtitles_temp["items"]:
                    if language["audio_exclude"] == "True":
                        if language_from_alpha2(
                            language["language"]
                        ) in ast.literal_eval(episode_subtitles["audio_language"]):
                            continue
                    desired_subtitles_list.append(
                        [language["language"], language["forced"], language["hi"]]
                    )
            ...
            if episode_subtitles["subtitles"] is not None:
                if use_embedded_subs:
                    ...
                    actual_subtitles_temp = [
                        x
                        for x in ast.literal_eval(episode_subtitles["subtitles"])
                        if x[1]
                    ]
            ...
            if cutoff_temp_list:
                for cutoff_temp in cutoff_temp_list:
                    ...
                    if cutoff_temp["audio_exclude"] == "True" and language_from_alpha2(
                        cutoff_temp["language"]
                    ) in ast.literal_eval(episode_subtitles["audio_language"]):
                        cutoff_met = True
        ...
        TableEpisodes.update(
            ...
            TableEpisodes.sonarrEpisodeId == episode_subtitles["sonarrEpisodeId"]
        ).execute()
