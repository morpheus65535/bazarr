...
def refine_from_ffprobe(path, video):
    ...
    if not data['ffprobe']:
        logging.debug(f"No FFprobe available in cache for this file: {path}")
        return video
    ...
    if 'video' not in data['ffprobe']:
        ...
        if 'resolution' in data['ffprobe']['video'][0] and not video.resolution:
            video.resolution = data['ffprobe']['video'][0]['resolution']
        if 'codec' in data['ffprobe']['video'][0] and not video.video_codec:
            video.video_codec = data['ffprobe']['video'][0]['codec']
        if 'frame_rate' in data['ffprobe']['video'][0] and not video.fps:
    ...
    if 'audio' not in data['ffprobe']:
        ...
        if 'codec' in data['ffprobe']['audio'][0] and not video.audio_codec:
