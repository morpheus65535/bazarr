...
def track_event(category=None, action=None, label=None):
    ...
    try:
        if os.path.isfile(
            os.path.normpath(os.path.join(args.config_dir, "config", "analytics.dat"))
        ):
            with open(
                os.path.normpath(
    ...
    tracker.add_custom_variable(
        CustomVariable(
            index=2, name="PythonVersion", value=platform.python_version(), scope=1
        )
    )
    if settings.general.getboolean("use_sonarr"):
        tracker.add_custom_variable(
            CustomVariable(index=3, name="SonarrVersion", value=sonarr_version, scope=1)
        )
    else:
        tracker.add_custom_variable(
            CustomVariable(index=3, name="SonarrVersion", value="unused", scope=1)
        )
    if settings.general.getboolean("use_radarr"):
        tracker.add_custom_variable(
            CustomVariable(index=4, name="RadarrVersion", value=radarr_version, scope=1)
        )
    else:
        tracker.add_custom_variable(
            CustomVariable(index=4, name="RadarrVersion", value="unused", scope=1)
        )
    tracker.add_custom_variable(
        CustomVariable(index=5, name="OSVersion", value=platform.platform(), scope=1)
    )
