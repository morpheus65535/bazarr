...
def call_autopulse_webhook(subtitle_path, media_path, language, media_type):
    ...
    if not (settings.general.use_autopulse or settings.plex.use_autopulse):
        return
...
def generate_complete_autopulse_config():
    """
    Generate complete Autopulse YAML configuration for copy-paste setup.
    Returns YAML string with triggers, targets, and auth configured.
    """
    try:
        ...
        # Generate complete YAML configuration (without auth section)
        # Auth fields in UI are for Bazarr->Autopulse communication only
        yaml_config = f"""# Autopulse Configuration - Generated by Bazarr
# Save this as config.toml in your Autopulse data directory
...
"""
        ...
        logging.info(f"BAZARR generated complete Autopulse config for server: {server_name}")
        return {
    ...
    try:
        ...
        for bazarr_pattern, plex_pattern in common_rewrites:
            if movie_path and bazarr_pattern in movie_path:
                detected = True
                suggestion = f"Detected path pattern '{bazarr_pattern}' in Bazarr settings. You may need rewriting."
                trigger_section = f"""
