...
def backup_to_zip():
    ...
    try:
        ...
        logging.exception(
            f"Unable to delete temporary database backup file: {database_backup_file}"
        )
...
def restore_from_backup():
    ...
    if os.path.isfile(restore_config_path) and os.path.isfile(restore_database_path):
        try:
            ...
            logging.exception(
                f"Unable to restore or delete config.ini to {dest_config_path}"
            )
        ...
        try:
            restart_file = io.open(
                os.path.join(args.config_dir, "bazarr.restart"), "w", encoding="UTF-8"
            )
...
def prepare_restore(filename):
    ...
    try:
        ...
        try:
            ...
            logging.exception(
                f"Unable to extract files from backup archive {dest_zip_file_path}"
            )
...
def backup_rotation():
    ...
    for file in backup_files:
        if (
            ...
            + timedelta(days=int(backup_retention))
            < datetime.utcnow()
        ):
