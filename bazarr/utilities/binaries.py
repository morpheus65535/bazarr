...
def get_binary(name):
    ...
    if installed_exe and os.path.isfile(installed_exe):
        ...
        if os.path.isfile(exe) and md5(exe) == binary["checksum"]:
            logging.debug(
                f"BAZARR returning this existing and up-to-date binary: {exe}"
            )
            return exe
        else:
            try:
                ...
                logging.debug(
                    "BAZARR downloading {0} from {1}".format(name, binary["url"])
                )
                ...
                logging.exception(
                    "BAZARR unable to download {0} to {1}".format(name, exe_dir)
                )
