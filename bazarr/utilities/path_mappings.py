...
class PathMappings:
    ...
    def update(self):
        ...
        self.path_mapping_sports = [x for x in settings.general.path_mappings_sports if x[0] != x[1]]
    ...
    def path_replace_sports(self, path):
        if path is None:
            return None

        for path_mapping in self.path_mapping_sports:
            if path_mapping[0] == path_mapping[1]:
                continue
            if '' in path_mapping:
                continue
    ...
    def path_replace_reverse_sports(self, path):
        if path is None:
            return None
