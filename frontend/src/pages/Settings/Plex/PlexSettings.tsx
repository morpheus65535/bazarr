...
export const PlexSettings: React.FC = () => {
  ...
  const getBestConnection = useCallback((connections: PlexServerConnection[]): PlexServerConnection | null => {
    const availableConnections = connections.filter((c) => c.available !== false);
  ...
  // Server selection
  const handleServerSelect = useCallback(async (machineIdentifier: string) => {
    const server = servers.find((s: PlexServer) => s.machineIdentifier === machineIdentifier);
            ...
            <Stack gap="sm">
              {pinMutation.data && !pinCheckData?.authenticated && (
                <>
                  ...
                  <Text>
                    PIN Code: <Text component="span" fw={700}>{pinMutation.data.code}</Text>
                  </Text>
                  <Text size="sm">Complete the authentication in the opened window.</Text>
                  <Button onClick={handleCancelAuth} variant="light" color="gray" size="sm">
                    Cancel
                  </Button>
                </>
              )}
            ...
            {servers.length === 0 ? (
              ...
              <Stack gap="md">
                <Group justify="space-between" align="center">
                  ...
                  {selectedServer?.machineIdentifier === servers[0].machineIdentifier ? (
                ...
                {servers[0].bestConnection && selectedServer?.machineIdentifier !== servers[0].machineIdentifier && (
                  <Button
                    onClick={() => handleServerSelect(servers[0].machineIdentifier)}
              ...
              <Group className={styles.serverSelectGroup}>
                <Select
                  ...
                  value={selectedServerId || selectedServer?.machineIdentifier || null}
                  onChange={(value: string | null) => setSelectedServerId(value)}
                ...
                <Button
                  ...
                  loading={serverSelectionMutation.isPending}
                  onClick={() => selectedServerId && handleServerSelect(selectedServerId)}
                >
                  Select Server
                </Button>
                <ActionIcon
