...
class SubsSabBzProvider(Provider):
    ...
    def process_archive_subtitle_files(self, archiveStream, language, video):
        ...
        for f in archiveStream.infolist():
            if f.filename.lower().endswith(('.srt', '.sub')):
                logger.info('Found subtitle file %r', f.filename)
                subtitle = SubsSabBzSubtitle(language, f.filename, type)
                subtitle.content = archiveStream.read(f)
