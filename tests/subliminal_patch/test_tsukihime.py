...
@pytest.mark.parametrize(
    ...
    [
        # TsukiHime exposes no uploader, so the streaming source of the release is surfaced there
        # to tell apart same-group variants such as Crunchyroll and Netflix WEB-DLs.
...
def test_streaming_service_is_exposed_as_uploader(episode, requests_mock, release_name, expected):
    _mock_anime(requests_mock, episode)
    requests_mock.get(
        f"{API}/animes/2086/episodes/1171",
        json={
    ...
    assert len(subtitles) == 1
    assert subtitles[0].uploader == expected


def test_same_release_tracks_are_distinguished_by_track_name(episode, requests_mock):
    # [Sonomama]-style releases bundle several full tracks for the same language; the track name is
    # what tells them apart, and it must not be dropped as if the two rows were duplicates.
    _mock_anime(requests_mock, episode)
    requests_mock.get(
        f"{API}/animes/2086/episodes/1171",
        json={
    ...
    assert {s.uploader for s in subtitles} == {"[Sonomama]", "Crunchyroll", None}
    assert len({s.id for s in subtitles}) == 3
